...
class user{
   ...
   const QUERY_USER_UPLOADS_SUMMARY = 'select img_media_type as type, DATE_FORMAT(img_timestamp,"%Y-%m") as created_month, sum(img_size) as somme, count(img_name) as compte from image , actor where img_actor = actor_id and actor_name = ? group by img_media_type, created_month order by created_month;';
   const QUERY_USER_ACTIONS = 'select log_type, count(*) as compte from logging, actor where log_actor = actor_id and actor_name =? group by log_type;';
