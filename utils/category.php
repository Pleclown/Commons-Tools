...
const QUERY_UPLOADERS_IN_CAT = 'select distinct img_user_text, count(img_name) as compte from image, page, categorylinks where page_namespace = 6 and img_name = page_title and page_id = cl_from and cl_to in (?) group by img_user order by count(img_name) desc';
